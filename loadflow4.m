clear; clc;

nBus = 33;
Vbase = 12660; % Volts
Sbase = 1e6;   % VA
Zbase = Vbase^2 / Sbase;

% Loads in per unit
Ppu = [0,100,90,120,60,60,200,200,60,60,45,60,60,120,60,60,60,90,90,90,90,90,90,420,420,60,60,60,120,200,150,210,60] / 1000;
Qpu = [0,60,40,80,30,20,100,100,20,20,30,35,35,80,10,20,20,40,40,40,40,40,50,200,200,25,25,20,70,600,70,100,40] / 1000;
% Now in MW and MVAR (i.e. per unit on 1MVA base)

branch = [
    1,2,0.0922,0.0470; 2,3,0.4930,0.2511; 3,4,0.3660,0.1864;
    4,5,0.3811,0.1941; 5,6,0.8190,0.7070; 6,7,0.1872,0.6188;
    7,8,0.7114,0.2351; 8,9,1.0300,0.7400; 9,10,1.0440,0.7400;
    10,11,0.1966,0.0650; 11,12,0.3744,0.1238; 12,13,1.4680,1.1550;
    13,14,0.5416,0.7129; 14,15,0.5910,0.5260; 15,16,0.7463,0.5450;
    16,17,1.2890,1.7210; 17,18,0.7320,0.5740; 2,19,0.1640,0.1565;
    19,20,1.5042,1.3554; 20,21,0.4095,0.4784; 21,22,0.7089,0.9373;
    3,23,0.4512,0.3083; 23,24,0.8980,0.7091; 24,25,0.8960,0.7011;
    6,26,0.2030,0.1034; 26,27,0.2842,0.1447; 27,28,1.0590,0.9337;
    28,29,0.8042,0.7006; 29,30,0.5075,0.2585; 30,31,0.9744,0.9630;
    31,32,0.3105,0.3619; 32,33,0.3410,0.5302;
];

% Convert branch impedances to per unit
branch(:,3) = branch(:,3) / Zbase;
branch(:,4) = branch(:,4) / Zbase;

nBranch = size(branch,1);
V = ones(nBus,1);

for iter = 1:100
    V_old = V;
    
    % Backward sweep
    Ibr = zeros(nBranch,1);
    for k = nBranch:-1:1
        t = branch(k,2);
        S = Ppu(t) + 1j*Qpu(t);
        Ibr(k) = conj(S / V(t));
        for m = 1:nBranch
            if branch(m,1) == t
                Ibr(k) = Ibr(k) + Ibr(m);
            end
        end
    end
    
    % Forward sweep
    V(1) = 1.0;
    for k = 1:nBranch
        f = branch(k,1);
        t = branch(k,2);
        Z = branch(k,3) + 1j*branch(k,4);
        V(t) = V(f) - Z*Ibr(k);
    end
    
    if max(abs(abs(V)-abs(V_old))) < 1e-6
        fprintf('Converged in %d iterations\n', iter);
        break
    end
end

Vmag = abs(V);

fprintf('\nBus Voltages:\n');
for i = 1:nBus
    fprintf('Bus %2d: %.4f pu\n', i, Vmag(i));
end

figure;
plot(1:nBus, Vmag, 'b-o', 'LineWidth',2, 'MarkerFaceColor','b');
set(gca, 'XTick', 1:33);
xlabel('Bus Number');
ylabel('Voltage (pu)');
title('IEEE 33-Bus Load Flow - Voltage Profile');
grid on;
yline(0.95,'r--','0.95 pu limit');
ylim([0.85 1.05]);
xlim([1 33]);